...
import { isProjectOverdue, paymentStatusLabels } from "@/features/project/project-status";
    ...
    <>
      <div className="flex flex-1 flex-col gap-5 overflow-x-hidden px-6 py-5 md:px-8">
        ...
        <Sheet open={open} onOpenChange={(value) => {
          ...
          <Button size="icon" className="fixed right-6 bottom-6 z-[60] size-12 rounded-full shadow-lg" aria-label={open ? "Tutup panel Project" : "Tambah Project"} onClick={() => setOpen((current) => !current)}>
            <Plus className={open ? "rotate-45 transition-transform duration-500" : "transition-transform duration-500"} />
          </Button>
          <SheetContent side="right" className="w-full sm:max-w-md">
            ...
            <form onSubmit={submit} className="flex flex-col gap-4 px-6 pb-6">
              ...
              <RichTextEditor value={form.brief ?? ""} onChange={(brief) => setForm({ ...form, brief })} />
              ...
              <TableBody>
                {projects.map((project, index) => (
                  <TableRow key={project.id} className="transition-colors hover:bg-muted/40">
                    ...
                    <TableCell><ProjectActions project={project} onView={setDetailProject} onDelete={setDeleting} onOrder={setOrderProject} /></TableCell>
                  </TableRow>
                ))}
