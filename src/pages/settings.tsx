...
export default function SettingsPage() {
  ...
  return <div className="flex flex-1 flex-col gap-6 px-6 py-5 md:px-8">
    <div className="flex flex-col gap-1"><h1 className="text-2xl font-semibold">Pengaturan</h1><p className="text-muted-foreground">Atur workspace, identitas dokumen, kategori, dan keamanan data Rakit.</p></div>
    <div className="grid gap-4 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Tampilan</CardTitle><CardDescription>Pilih tema yang paling nyaman. Pilihan diterapkan dan disimpan otomatis.</CardDescription></CardHeader><CardContent className="grid grid-cols-2 gap-2"><Button variant={theme === "light" ? "default" : "outline"} onClick={() => changeTheme("light")}><Sun data-icon="inline-start" />Terang</Button><Button variant={theme === "dark" ? "default" : "outline"} onClick={() => changeTheme("dark")}><Moon data-icon="inline-start" />Gelap</Button></CardContent></Card>
      <Card><CardHeader><CardTitle>Folder Utama Project</CardTitle><CardDescription>Dipakai untuk project dan Arsip baru. Project yang telah ada tidak dipindahkan.</CardDescription></CardHeader><CardContent className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]"><Input value={projectsRoot} onChange={(event) => setProjectsRoot(event.target.value)} aria-label="Lokasi folder utama project" /><Button size="icon" variant="outline" onClick={() => void chooseRoot()} aria-label="Pilih folder utama"><FolderOpen /></Button><Button onClick={() => void saveRoot()}><Save data-icon="inline-start" />Simpan</Button></CardContent></Card>
      <Card className="lg:col-span-2"><CardHeader><CardTitle>Identitas Agency</CardTitle><CardDescription>Digunakan untuk Invoice dan Nota baru.</CardDescription></CardHeader><CardContent className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto]"><div className="grid gap-2"><label className="text-sm font-medium" htmlFor="agency-name">Nama agency</label><Input id="agency-name" value={agencyName} onChange={(event) => setAgencyName(event.target.value)} placeholder="Nama agency" /></div><div className="flex flex-wrap items-end gap-2"><Button type="button" variant="outline" onClick={() => void chooseLogo()}><Image data-icon="inline-start" />Pilih Logo</Button><Badge variant="secondary">{logoPath ? "Logo dipilih" : "Tanpa logo"}</Badge><Button onClick={() => void saveAgencyIdentity()} disabled={!invoiceSettings}><Save data-icon="inline-start" />Simpan</Button></div></CardContent></Card>
      <Card><CardHeader><CardTitle>Kategori Idea</CardTitle><CardDescription>Kategori preset dijaga tetap ada. Kategori tambahan dapat dihapus kapan saja.</CardDescription></CardHeader><CardContent className="grid gap-4"><div className="flex flex-col gap-2 sm:flex-row"><Input value={name} onChange={(event) => setName(event.target.value)} onKeyDown={(event) => event.key === "Enter" && (event.preventDefault(), addCategory())} placeholder="Kategori baru" /><Button onClick={addCategory}><Plus data-icon="inline-start" />Tambah</Button></div><div className="flex flex-wrap gap-2">{categories.map((category) => <Badge key={category} variant={presetIdeaCategories.includes(category) ? "secondary" : "outline"} className="gap-1 py-1.5">{category}{!presetIdeaCategories.includes(category) && <button type="button" aria-label={`Hapus ${category}`} onClick={() => void saveCategories(categories.filter((item) => item !== category))}><Trash2 className="size-3" /></button>}</Badge>)}</div></CardContent></Card>
      <Card><CardHeader><CardTitle>Kategori Reference</CardTitle><CardDescription>Kelompokkan website referensi agar lebih mudah ditemukan kembali.</CardDescription></CardHeader><CardContent className="grid gap-4"><div className="flex flex-col gap-2 sm:flex-row"><Input value={referenceName} onChange={(event) => setReferenceName(event.target.value)} onKeyDown={(event) => event.key === "Enter" && (event.preventDefault(), addReferenceCategory())} placeholder="Kategori baru" /><Button onClick={addReferenceCategory}><Plus data-icon="inline-start" />Tambah</Button></div><div className="flex flex-wrap gap-2">{referenceCategories.map((category) => <Badge key={category} variant={presetReferenceCategories.includes(category) ? "secondary" : "outline"} className="gap-1 py-1.5">{category}{!presetReferenceCategories.includes(category) && <button type="button" aria-label={`Hapus ${category}`} onClick={() => void saveReferenceCategoryList(referenceCategories.filter((item) => item !== category))}><Trash2 className="size-3" /></button>}</Badge>)}</div></CardContent></Card>
      <Card className="border-primary/30 lg:col-span-2"><CardHeader><CardTitle>Backup Data</CardTitle><CardDescription>Simpan salinan data secara berkala. Import akan menggantikan seluruh data yang sedang ada setelah konfirmasi.</CardDescription></CardHeader><CardContent className="flex flex-wrap gap-2"><Button onClick={() => void exportBackup()} disabled={backupBusy}><Download data-icon="inline-start" />Export Backup</Button><Button variant="outline" onClick={() => void chooseBackup()} disabled={backupBusy}><Upload data-icon="inline-start" />Import Backup</Button></CardContent></Card>
    </div>
