...
export default function FinancePage() {
  ...
  return <main className="flex flex-1 flex-col gap-4 p-4 md:p-6"><div><h1 className="text-2xl font-semibold">Keuangan</h1><p className="text-muted-foreground">Pantau omset pesanan, pengeluaran, dan margin riil setiap project.</p></div><div className="grid gap-4 md:grid-cols-3"><Card><CardHeader><CardDescription>Omset</CardDescription><CardTitle className="text-2xl">{rupiah(income)}</CardTitle></CardHeader></Card><Card><CardHeader><CardDescription>Pengeluaran</CardDescription><CardTitle className="text-2xl">{rupiah(expense)}</CardTitle></CardHeader></Card><Card><CardHeader><CardDescription>Margin Riil · {summaries.length} project</CardDescription><CardTitle className="text-2xl">{rupiah(income - expense)}</CardTitle></CardHeader></Card></div><ProjectResults rows={summaries} /><div className="grid gap-4 xl:grid-cols-2"><History title="Riwayat Omset" rows={incomes} income /><History title="Riwayat Pengeluaran" rows={expenses} income={false} /></div>{!expenseOpen && <Button className="fixed right-6 bottom-6 z-[60] size-12 rounded-full shadow-lg" size="icon" onClick={() => setExpenseOpen(true)} disabled={!eligibleCount} aria-label="Input pengeluaran" title="Input pengeluaran"><Plus /></Button>}<ExpenseDialog open={expenseOpen} onOpenChange={setExpenseOpen} projects={summaries} onSaved={refresh} /></main>;
}
