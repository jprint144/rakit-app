...
function SortableWebsite({
  ...
  return (
    <Button
      ...
      title={`${item.title} — tarik untuk mengubah urutan`}
      onClick={() => onOpen(item.url)}
    >
      <img
        className="size-5 shrink-0 aspect-square rounded-sm object-contain"
...
export function ReferenceSubsidebar() {
  ...
  useEffect(() => {
    const updateHeaderState = (event: Event) =>
      setHeaderHidden((event as CustomEvent<boolean>).detail);
    window.addEventListener("reference:header-hidden", updateHeaderState);
    return () =>
      window.removeEventListener("reference:header-hidden", updateHeaderState);
  }, []);

  const open = (url: string) =>
    window.dispatchEvent(
      new CustomEvent("reference:navigate", { detail: url }),
    );
  ...
  const toggleFocusMode = () =>
    setFocusMode((value) => {
      const next = !value;
      window.dispatchEvent(
        new CustomEvent("reference:focus-mode", { detail: next }),
      );
      return next;
    });
  ...
  return (
    <aside
      className={cn(
        "flex shrink-0 flex-col border-r bg-card text-card-foreground py-3 transition-[width]",
        compact ? "w-14" : "w-52",
      )}
    >
      <div
      ...
      >
        {!compact && <p className="text-sm font-semibold">Website</p>}
        <div
          className={cn("flex", compact ? "flex-col gap-1" : "items-center")}
        >
          <Button
            size="icon"
            variant="ghost"
          ...
          </Button>
          {
            <Button
              ...
              variant="ghost"
              title={
                headerHidden ? "Tampilkan bar atas" : "Sembunyikan bar atas"
              }
              onClick={() =>
                window.dispatchEvent(
                  new Event(
                    headerHidden
                      ? "reference:show-header"
                      : "reference:hide-header",
                  ),
                )
              }
            >
              {headerHidden ? <ChevronDown /> : <ChevronUp />}
            </Button>
          }
          <Button
            size="icon"
          ...
          >
            {items.map((item, index) => (
              <div key={item.id} className="grid gap-1">
                ...
                <SortableWebsite compact={compact} item={item} onOpen={open} />
              </div>
            ))}
          </SortableContext>
        </div>
